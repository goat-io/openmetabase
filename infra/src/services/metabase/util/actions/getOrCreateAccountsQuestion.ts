async function getOrCreateAccountsQuestion({
  mbToken,
  collectionId,
  databaseId,
}: {
  ...
  console.log("🔍 Checking if question '# Accounts' exists...")
  ...
  if (existingQuestion) {
    console.log(
      `✅ Question '# Accounts' already exists (ID: ${existingQuestion.id}).`,
    )
    return existingQuestion.id
  }

  console.log("🛠 Creating question '# Accounts'...")
  ...
  if (createRes.ok) {
    ...
    console.log(`✅ Question '# Accounts' created (ID: ${questionData.id}).`)
